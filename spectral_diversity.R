...
for (i in plots$Nummer){
  ...
  write.csv(x = res_c, file = paste0("./results/metric_tables/Planet_site",i,"_metrics.csv"),
            row.names = F)
...
agg <- c(1, 2, 4)
for (i in sites){
  ls <- list.files("./data/_raster/original/",
                   pattern = paste0("site",i,"_resampled_georef_clipped_aligned.tif$"), full.names = T)
  for (j in agg){
    r <- lapply(ls, rast)
    r <- lapply(r, terra::aggregate, j)
    r <- lapply(r, crop, aoi_vec[aoi_vec$Nummer == i])
    ...
    v <- lapply(dist_rasters, values)
    m <- lapply(v, mean, na.rm = T)
    
    d <- data.frame(SSD = unlist(m),
                    date = c(20240408, 20240523, 20240722, 20240812))
    
    res <- read.csv(file = paste0("./results/metric_tables/UAS_site",i,
                                  "_agg",j,"_metrics.csv"))
    class(res$date)
    class(d$date)
    res_c <- left_join(res, d)
    write.csv(res_c, file = paste0("./results/metric_tables/UAS_site",i,
                                   "_agg",j,"_metrics.csv"),
              row.names = F)
    }
